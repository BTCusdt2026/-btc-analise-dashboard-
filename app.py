...
.card {padding: 18px; border-radius: 16px; border: 1px solid #30363D; margin-bottom: 15px; color: white;}
</style>
...
if not st.session_state["logado"]:
    ...
    try:
        ...
        fig.update_layout(template="plotly_dark", paper_bgcolor="#0E1117", plot_bgcolor="#0E1117", xaxis_rangeslider_visible=False, height=500)
        st.plotly_chart(fig, use_container_width=True)
        
        ultimo = df.iloc[-1]
        col1, col2, col3 = st.columns(3)
        col1.metric("RSI 14", f"{ultimo['rsi']:.1f}")
        col2.metric("MACD", "Bullish" if ultimo['macd'] > 0 else "Bearish")
        col3.metric("Volume", f"${ultimo['volume']:,.0f}")
        
        st.caption(f"Dados em tempo real do Yahoo Finance • {datetime.now().strftime('%H:%M:%S')}")
